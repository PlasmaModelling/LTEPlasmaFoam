...
void Foam::heheuPsiThermo<BasicPsiThermo, MixtureType>::calculate()
{
    ...
    forAll(TCells, celli)
    {
		...
		sigmaECells[celli] = mixture_.sigmaE(pCells[celli], TCells[celli]);
    ...
    volScalarField::Boundary& sigmaEBf =
		this->sigmaE_.boundaryFieldRef();
    ...
    forAll(this->T_.boundaryField(), patchi)
    {
        ...
        if (pT.fixesValue())
        {
            forAll(pT, facei)
            {
				...
				psigmaE[facei] = mixture_.sigmaE(pp[facei], pT[facei]);
            }
        }
        else
        {
...
template<class BasicPsiThermo, class MixtureType>
Foam::tmp<Foam::volScalarField>
Foam::heheuPsiThermo<BasicPsiThermo, MixtureType>::sigmaEu() const
{
	tmp<volScalarField> tsigmaEu
	(
		new volScalarField
		(
			IOobject
	...
	volScalarField& sigmaEu_ = tsigmaEu.ref();
	scalarField& sigmaEuCells = sigmaEu_.primitiveFieldRef();
	const scalarField& pCells = this->p_;
	const scalarField& TuCells = this->Tu_;
	
	forAll(sigmaEuCells, celli)
	{
		sigmaEuCells[celli] = this->cellReactants(celli).sigmaE
		(
			pCells[celli],
			TuCells[celli]
		);
	...
	volScalarField::Boundary& sigmaEuBf = sigmaEu_.boundaryFieldRef();
	
	forAll(sigmaEuBf, patchi)
	{
		fvPatchScalarField& pSigmaEu = sigmaEuBf[patchi];
		const fvPatchScalarField& pp = this->p_.boundaryField()[patchi];
		const fvPatchScalarField& pTu = this->Tu_.boundaryField()[patchi];
		
		forAll(pSigmaEu, facei)
		{
			pSigmaEu[facei] = this->patchFaceReactants(patchi, facei).sigmaE
			(
				pp[facei],
				pTu[facei]
			);
...
template<class BasicPsiThermo, class MixtureType>
Foam::tmp<Foam::volScalarField>
Foam::heheuPsiThermo<BasicPsiThermo, MixtureType>::sigmaEb() const
{
	tmp<volScalarField> tsigmaEb
	(
		new volScalarField
		(
			IOobject
	...
	volScalarField& sigmaEb_ = tsigmaEb.ref();
	scalarField& sigmaEbCells = sigmaEb_.primitiveFieldRef();
	const volScalarField Tb_(Tb());
	const scalarField& pCells = this->p_;
	const scalarField& TbCells = Tb_;
	
	forAll(sigmaEbCells, celli)
	{
		sigmaEbCells[celli] = this->cellProducts(celli).sigmaE
		(
			pCells[celli],
			TbCells[celli]
		);
	...
	volScalarField::Boundary& sigmaEbBf = sigmaEb_.boundaryFieldRef();
	
	forAll(sigmaEbBf, patchi)
	{
		fvPatchScalarField& pSigmaEb = sigmaEbBf[patchi];
		const fvPatchScalarField& pp = this->p_.boundaryField()[patchi];
		const fvPatchScalarField& pTb = Tb_.boundaryField()[patchi];
		
		forAll(pSigmaEb, facei)
		{
			pSigmaEb[facei] = this->patchFaceProducts(patchi, facei).sigmaE
			(
				pp[facei],
				pTb[facei]
			);
