...
:
	...
	(
		IOobject
		(
			phasePropertyName("thermo:sigmaE"),
			mesh.time().timeName(),
			mesh,
			IOobject::NO_READ,
			IOobject::NO_WRITE
...
Foam::tmp<Foam::volScalarField> Foam::psiThermo::sigmaE() const
{
	return sigmaE_;
}


Foam::tmp<Foam::scalarField> Foam::psiThermo::sigmaE(const label patchi) const
{
